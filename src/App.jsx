...
function IconBox({ name, active = false }) {
  const activeClass = "bg-gradient-to-br from-orange-500 via-red-500 to-fuchsia-600 border-orange-300/70 text-white shadow-[0_0_22px_rgba(249,115,22,0.35)]";
  const idleClass = "bg-white/[0.04] border-white/10 text-zinc-300 group-hover:text-orange-200 group-hover:border-orange-300/40 group-hover:bg-orange-500/10";
  ...
  if (compact) {
    return (
      <div className="flex items-center gap-3">
        <div className="bg-gradient-to-br from-orange-500 via-red-500 to-fuchsia-600 text-white rounded-2xl h-11 w-11 flex items-center justify-center text-xs font-black border border-orange-300/50 shadow-[0_0_24px_rgba(249,115,22,0.34)]">CS2</div>
        <div className="leading-none">
          <div className="text-white font-black text-2xl tracking-tight">DNG</div>
          <div className="text-[10px] font-black tracking-[0.3em] text-orange-200/70">EU</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-md">
      <div className={`w-full max-w-xl rounded-3xl border ${theme.border} ${theme.panel} p-7 text-white ${theme.glow}`}>
        ...
        <div className="mt-6 rounded-2xl border border-orange-950/60 bg-black/40 p-5">
          <div className="flex items-end justify-between"><span className="text-zinc-400">Игроки</span><b className="text-4xl text-orange-300">{searchState.players}/10</b></div>
          <div className="mt-4 h-3 rounded-full bg-zinc-800"><div className="h-3 rounded-full bg-gradient-to-r from-orange-400 to-red-500 transition-all" style={{ width: `${searchState.players * 10}%` }} /></div>
          <p className="mt-4 text-zinc-300">{searchState.message}</p>
        </div>
        {searchState.match && <div className="mt-5 grid sm:grid-cols-3 gap-3 text-center">{[[searchState.match.map, "Карта"], [searchState.match.server, "Сервер"], [`${searchState.match.eloChange > 0 ? "+" : ""}${searchState.match.eloChange}`, "ELO"]].map(([value, label]) => <div key={label} className="rounded-2xl border border-orange-950/60 bg-black/40 p-4"><b className="text-xl">{value}</b><p className="text-xs text-zinc-400">{label}</p></div>)}</div>}
      </div>
    </div>
  );
}
...
function StatCard({ value, label }) {
  return <div className={`${theme.card} rounded-2xl border border-orange-950/50 p-4 text-center`}><b className="text-2xl text-orange-200">{value}</b><p className="text-xs text-zinc-400">{label}</p></div>;
}
...
function AuthPage({ mode, setPage, language, users, setUsers, setCurrentUser }) {
  ...
  return <main className="flex-1 overflow-auto bg-transparent text-white p-4 sm:p-6 lg:p-10"><div className="mx-auto max-w-xl rounded-3xl border border-orange-500/25 bg-black/60 p-6 lg:p-8 shadow-[0_0_42px_rgba(255,90,20,0.20)] backdrop-blur-xl"><Icon name={isRegister ? "plus" : "login"} size={46} className="text-orange-400" /><h1 className="mt-4 text-3xl lg:text-4xl font-black">{isRegister ? t(language, "register") : t(language, "login")}</h1><p className="mt-2 text-zinc-400">{isRegister ? "Создай аккаунт Dropnetgaming. Данные сохраняются в браузере." : "Войди в аккаунт Dropnetgaming."}</p><div className="mt-7 space-y-4"><TextInput label="Логин" value={username} onChange={setUsername} placeholder="username" />{isRegister && <TextInput label="Никнейм" value={nickname} onChange={setNickname} placeholder="skwizzy22" />}{isRegister && <TextInput label="Email" value={email} onChange={setEmail} placeholder="mail@example.com" />}<TextInput label="Пароль" value={password} onChange={setPassword} type="password" placeholder="password" />{error && <div className="rounded-xl border border-red-500/40 bg-red-500/10 p-3 text-red-200">{error}</div>}<div className="flex flex-col sm:flex-row gap-3"><ActionButton onClick={submit}>{isRegister ? "Зарегистрироваться" : "Войти"}</ActionButton><ActionButton dark onClick={() => goTo(setPage, isRegister ? "login" : "register")}>{isRegister ? "Уже есть аккаунт" : "Создать аккаунт"}</ActionButton></div></div></div></main>;
}
...
function AdminPage({ currentUser, users, setUsers, tickets, setTickets, news, setNews, setPage }) {
  ...
  if (!currentUser || currentUser.role !== "admin") return <main className="flex-1 overflow-auto bg-transparent text-white p-4 sm:p-6 lg:p-10"><div className="mx-auto max-w-2xl rounded-3xl border border-orange-500/25 bg-black/60 p-8 backdrop-blur-xl"><Icon name="admin" size={48} className="text-orange-400" /><h1 className="mt-4 text-4xl font-black">Админ-панель</h1><p className="mt-3 text-zinc-400">Доступ только для администратора. Войди в аккаунт администратора.</p><div className="mt-6"><ActionButton onClick={() => goTo(setPage, "login")}>Войти</ActionButton></div></div></main>;
  ...
  return <main className="flex-1 overflow-auto bg-transparent text-white p-4 sm:p-6 lg:p-10"><div className="max-w-7xl mx-auto space-y-6"><section className={`rounded-3xl ${theme.panel} border ${theme.border} p-6 lg:p-8 ${theme.glow}`}><Icon name="admin" size={50} className="text-orange-400" /><h1 className="mt-3 text-3xl lg:text-4xl font-black">Админ-панель Dropnetgaming</h1><p className="mt-2 text-zinc-400">Пользователи, новости, обращения и роли.</p>{notice && <div className="mt-4 rounded-xl border border-orange-400/30 bg-orange-500/10 p-3 text-orange-100">{notice}</div>}</section><section className="grid sm:grid-cols-2 xl:grid-cols-4 gap-4">{[[users.length, "Пользователей"], [users.filter((user) => user.role === "admin").length, "Админов"], [tickets.length, "Обращений"], [news.length, "Новостей"]].map(([value, label]) => <StatCard key={label} value={value} label={label} />)}</section><section className={`${theme.panel} rounded-3xl border ${theme.border} p-6`}><h2 className="text-2xl font-black mb-4">Добавить новость</h2><div className="grid lg:grid-cols-[1fr_1fr_160px_140px] gap-3"><input value={newTitle} onChange={(e) => setNewTitle(e.target.value)} placeholder="Заголовок" className="rounded-2xl border border-orange-950/60 bg-black/55 px-4 py-3 outline-none focus:border-orange-400" /><input value={newText} onChange={(e) => setNewText(e.target.value)} placeholder="Текст новости" className="rounded-2xl border border-orange-950/60 bg-black/55 px-4 py-3 outline-none focus:border-orange-400" /><input value={newTag} onChange={(e) => setNewTag(e.target.value)} placeholder="Тег" className="rounded-2xl border border-orange-950/60 bg-black/55 px-4 py-3 outline-none focus:border-orange-400" /><label className="flex items-center gap-2 rounded-2xl border border-orange-950/60 bg-black/55 px-4 py-3"><input type="checkbox" checked={newHot} onChange={(e) => setNewHot(e.target.checked)} /> HOT</label></div><div className="mt-4"><ActionButton onClick={addNews}>Опубликовать новость</ActionButton></div><div className="mt-5 grid gap-3">{news.map((item) => <div key={item.id} className="rounded-2xl border border-orange-950/60 bg-black/35 p-4"><div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2"><b>{item.title}</b><button type="button" onClick={() => deleteNews(item.id)} className="rounded-xl border border-red-400/30 px-3 py-2 hover:bg-red-500/20">Удалить</button></div><p className="mt-2 text-zinc-300">{item.text}</p><p className="mt-2 text-xs text-zinc-500">{item.tag} • {item.createdAt}</p></div>)}</div></section><section className={`${theme.panel} rounded-3xl border ${theme.border} p-6`}><h2 className="text-2xl font-black mb-4">Обращения</h2><div className="grid gap-3">{tickets.length === 0 && <p className="text-zinc-400">Пока обращений нет.</p>}{tickets.map((ticket) => <div key={ticket.id} className="rounded-2xl border border-orange-950/60 bg-black/35 p-4"><div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2"><b>{ticket.subject}</b><div className="flex gap-2"><select value={ticket.status} onChange={(e) => updateTicketStatus(ticket.id, e.target.value)} className="rounded-xl border border-orange-950/60 bg-black/70 px-3 py-2"><option>Новое</option><option>В работе</option><option>Закрыто</option><option>Отклонено</option></select><button type="button" onClick={() => deleteTicket(ticket.id)} className="rounded-xl border border-red-400/30 px-3 py-2 hover:bg-red-500/20">Удалить</button></div></div><p className="mt-2 text-zinc-300">{ticket.message}</p><p className="mt-3 text-xs text-zinc-500">{ticket.author} • {ticket.createdAt} • {ticket.status}</p></div>)}</div></section><section className={`${theme.panel} rounded-3xl border ${theme.border} p-6 overflow-x-auto`}><h2 className="text-2xl font-black mb-4">Аккаунты</h2><table className="w-full min-w-[820px] text-left text-sm"><thead className="bg-orange-500/10 text-orange-200"><tr><th className="p-4">Логин</th><th className="p-4">Ник</th><th className="p-4">ELO</th><th className="p-4">Level</th><th className="p-4">Роль</th><th className="p-4">Действия</th></tr></thead><tbody>{users.map((user) => <tr key={user.username} className="border-t border-orange-950/40"><td className="p-4 font-bold">{user.username}</td><td className="p-4">{user.nickname || user.username}</td><td className="p-4 text-orange-200 font-black">{user.elo}</td><td className="p-4">{user.level}</td><td className="p-4"><span className={`rounded-full px-3 py-1 text-xs font-black ${user.role === "admin" ? "bg-orange-500/20 text-orange-200" : "bg-white/10 text-zinc-300"}`}>{user.role}</span></td><td className="p-4 flex gap-2"><button type="button" onClick={() => makeAdmin(user.username)} className="rounded-xl border border-orange-400/30 px-3 py-2 hover:bg-orange-500/20">Админ</button><button type="button" onClick={() => deleteUser(user.username)} className="rounded-xl border border-red-400/30 px-3 py-2 hover:bg-red-500/20">Удалить</button></td></tr>)}</tbody></table></section></div></main>;
}
...
function MissionsPage({ quests }) {
  return <main className="flex-1 overflow-auto bg-transparent text-white p-4 sm:p-6 lg:p-10"><div className="max-w-5xl mx-auto"><section className={`rounded-3xl ${theme.panel} border ${theme.border} p-6 lg:p-8 ${theme.glow}`}><Icon name="missions" size={48} className="text-orange-400" /><h1 className="text-4xl font-black mt-4">Ежедневные задания</h1><p className="text-zinc-400 mt-2">Задания автоматически обновляются каждый день. За выполнение начисляется XP.</p></section><div className="grid md:grid-cols-3 gap-5 mt-6">{quests.quests.map((quest) => <div key={quest.id} className={`${theme.card} rounded-3xl border border-orange-950/60 p-6`}><div className="flex items-center justify-between"><Icon name="trophy" className="text-orange-400" /><span className={quest.completed ? "text-green-400 font-black" : "text-zinc-400 font-black"}>{quest.completed ? "ГОТОВО" : `${quest.progress}/${quest.goal}`}</span></div><h2 className="text-xl font-black mt-5">{quest.title}</h2><div className="mt-4 h-2 rounded-full bg-zinc-800"><div className="h-2 rounded-full bg-gradient-to-r from-orange-400 to-red-500" style={{ width: `${Math.min(100, (quest.progress / quest.goal) * 100)}%` }} /></div><p className="mt-4 text-orange-200 font-black">+{quest.reward} XP</p></div>)}</div></div></main>;
}
...
function LeaderboardPage({ users, currentUser }) {
  ...
  return <main className="flex-1 overflow-auto bg-transparent text-white p-4 sm:p-6 lg:p-10"><div className="max-w-6xl mx-auto"><section className={`rounded-3xl ${theme.panel} border ${theme.border} p-6 lg:p-8 ${theme.glow}`}><Icon name="chart" size={48} className="text-orange-400" /><h1 className="text-4xl font-black mt-4">Лидерборд</h1><p className="text-zinc-400 mt-2">Топ игроков по ELO. Твой аккаунт тоже попадает в таблицу.</p></section><section className={`${theme.panel} rounded-3xl border ${theme.border} p-4 sm:p-6 mt-6 overflow-x-auto`}><table className="w-full min-w-[720px] text-left"><thead className="text-orange-200 bg-orange-500/10"><tr><th className="p-4">#</th><th className="p-4">Игрок</th><th className="p-4">ELO</th><th className="p-4">Level</th><th className="p-4">Победы</th><th className="p-4">Матчи</th></tr></thead><tbody>{leaders.map((user, index) => <tr key={`${user.username}-${index}`} className={`border-t border-orange-950/50 ${currentUser?.username === user.username ? "bg-orange-500/10" : ""}`}><td className="p-4 font-black text-orange-300">#{index + 1}</td><td className="p-4"><div className="flex items-center gap-3"><Avatar seed={user.nickname || user.username} /><b>{user.nickname || user.username}</b></div></td><td className="p-4 font-black">{user.elo}</td><td className="p-4">{user.level}</td><td className="p-4">{user.wins}</td><td className="p-4">{user.matches}</td></tr>)}</tbody></table></section></div></main>;
}
...
export default function DropnetgamingApp() {
  ...
  return <div className="relative min-h-screen lg:h-screen overflow-auto lg:overflow-hidden font-sans" style={backgroundStyle}><div className="pointer-events-none absolute inset-0 opacity-35 bg-[linear-gradient(rgba(255,255,255,0.035)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.035)_1px,transparent_1px)] bg-[size:44px_44px]" /><div className="pointer-events-none absolute left-[-120px] top-[-120px] h-80 w-80 rounded-full bg-orange-500/20 blur-3xl" /><div className="pointer-events-none absolute right-[-100px] bottom-[-140px] h-96 w-96 rounded-full bg-fuchsia-600/15 blur-3xl" /><MatchSearchModal searchState={searchState} onCancel={() => setSearchState({ active: false, found: false, players: 0, message: "", match: null })} /><div className="relative z-10 min-h-screen lg:h-full flex flex-col"><TopNav page={page} setPage={setPage} language={language} currentUser={currentUser} onLogout={onLogout} notifications={notifications} /><div className="flex-1 flex flex-col lg:flex-row bg-transparent overflow-visible lg:overflow-hidden"><LeftSidebar page={page} setPage={setPage} language={language} /><CurrentPage /><RightBar setPage={setPage} language={language} /></div></div></div>;
}
