      ...
      <section className="modal-card">
        ...
        <div className="modal-progress">
          ...
          <div className="progress">
            <span style={{ width: `${state.players * 10}%` }} />
          </div>
        ...
        {state.match && (
          <div className="modal-stats">
            ...
            <span>
              <b>{state.match.server}</b>
              Сервер
            </span>

            <span>
              <b>
                {state.match.eloChange > 0 ? "+" : ""}
                {state.match.eloChange}
              </b>
              ELO
            </span>
          </div>
...
export default function App() {
  ...
  function updateCurrentUser(updated) {
    ...
    setUsers((prev) =>
      prev.map((user) => (user.username === normalized.username ? normalized : user))
    );
  }
  ...
  function finishMatch() {
    ...
    const score = win
      ? `13-${7 + Math.floor(Math.random() * 5)}`
      : `${8 + Math.floor(Math.random() * 4)}-13`;
